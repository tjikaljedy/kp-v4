...
public class BsonQueryFactory implements QueryFactory {
	...
	/**
	 * The marshall operator will be replacing the token during query parsing as
	 * following: {"firstname":#} -> {"firstname":{MARSHALL_OPERATOR: 0}} 0 being
	 * the index of the parameter to be inserted in place of that placeholder.
	 * Previously $marshall but upgrading to mongo driver 4 the new parser does not
	 * allow $ prefixed strings if they're not mongo operators. With a UUID prefixed
	 * string there should be no risk of collision.
	 */
	...
	public Query createQuery(final String query, Object... parameters) {
		...
		try {
			if (quotedQuery.charAt(0) == '[') {
				...
				// also we could do this for simple objects but it would not handle properly
				// queries like
				// "{'a':1}, {'b':1}" as tested in
	...
	private String addRequiredQuotesAndParameters(String query, Object[] parameters) {
		...
		for (char nextChar : query.toCharArray()) {
			if (ctxStack.peek() == Context.STRING) {
				...
				if ("$oid".equals(previousToken) || !isValueToken(query, position)) {
	...
	private String sanitize(Object parameter1, char currentStringStartingQuote) {
		return Objects.toString(parameter1).replaceAll(String.valueOf(currentStringStartingQuote),
				"\\\\" + currentStringStartingQuote);
	}
	...
	private void appendParamPlaceholder(StringBuilder result, int paramIndex) {
		result.append('{').append('"').append(MARSHALL_OPERATOR).append('"').append(':').append(paramIndex).append('}');
	}

	private StringBuilder trimAppendParamAndQuote(StringBuilder currentToken, Object parameter) {
		return new StringBuilder().append('"').append(currentToken.toString().trim()).append(sanitize(parameter, '"'))
				.append('"');
	}
	...
	/**
	 * The object may have been serialized to a primitive type with a custom
	 * serializer, so try again after wrapping as an object property. We do this
	 * trick only as a falllback since it causes Jackson to consider the parameter
	 * as "Object" and thus ignore any annotations that may exist on its actual
	 * class.
	 */
