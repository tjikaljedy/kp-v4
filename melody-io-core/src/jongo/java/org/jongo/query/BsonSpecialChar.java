...
enum BsonSpecialChar {
	...
	LEFT_CURLY_BRACE('{') {
		@Override
		String applySpecificBehaviour(StringBuilder result, StringBuilder currentToken, Stack<Context> ctxStack,
				int position) {
	...
	abstract String applySpecificBehaviour(StringBuilder result, StringBuilder currentToken, Stack<Context> ctxStack,
			int position);
	...
	private static String appendNextCharAndPopCurrentToken(StringBuilder result, StringBuilder currentToken,
			char nextChar) {
