...
public class MongoCollection {
	...
	private MongoCollection(DBCollection dbCollection, Mapper mapper, WriteConcern writeConcern,
			ReadPreference readPreference, ReadConcern readConcern) {
	...
	public FindOne findOne(ObjectId id) {
		...
		return new FindOne(collection, readPreference, mapper.getUnmarshaller(), mapper.getQueryFactory(), "{_id:#}",
				id);
	}
	...
	public WriteResult insert(Object... pojos) {
		return new Insert(collection, writeConcern, mapper.getMarshaller(), mapper.getObjectIdUpdater(),
				mapper.getQueryFactory()).insert(pojos);
	}

	public WriteResult insert(String query, Object... parameters) {
		return new Insert(collection, writeConcern, mapper.getMarshaller(), mapper.getObjectIdUpdater(),
				mapper.getQueryFactory()).insert(query, parameters);
	}
	...
	public Aggregate aggregate(String pipelineOperator, Object... parameters) {
		return new Aggregate(collection, mapper.getUnmarshaller(), mapper.getQueryFactory()).and(pipelineOperator,
				parameters);
	}
