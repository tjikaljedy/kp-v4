...
class BsonSerializers extends SimpleSerializers {
	...
	static class BSONTimestampSerializer extends JsonSerializer<BSONTimestamp> {
		...
		public void serialize(BSONTimestamp obj, JsonGenerator jsonGenerator, SerializerProvider serializerProvider)
				throws IOException, JsonProcessingException {
			if (jsonGenerator instanceof MongoBsonGenerator) {
	...
	static class Decimal128Serializer extends JsonSerializer<Decimal128> {
		...
		public void serialize(Decimal128 decimal, JsonGenerator jsonGenerator, SerializerProvider serializerProvider)
				throws IOException, JsonProcessingException {
			if (jsonGenerator instanceof MongoBsonGenerator) {
	...
	private static void handleUnsupportedGenerator(JsonGenerator jsonGenerator) {
		throw new IllegalArgumentException(
				"BsonSerializer can only be used with MongoBsonGenerator or TokenBuffer. Found "
						+ jsonGenerator.getClass().getName());
	}
